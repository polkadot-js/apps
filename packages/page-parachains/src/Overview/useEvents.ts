...
import type { PolkadotPrimitivesV9CandidateReceiptV2 } from '@polkadot/types/lookup';
...
function includeEntry (map: EventMap, event: Event, blockHash: string, blockNumber: BN): void {
  try {
    const { descriptor } = (event as unknown as IEvent<[PolkadotPrimitivesV9CandidateReceiptV2]>).data[0];
