...
import type { PolkadotPrimitivesV8CandidateReceipt } from '@polkadot/types/lookup';
...
function includeEntry (map: EventMap, event: Event, blockHash: string, blockNumber: BN): void {
  try {
    const { descriptor } = (event as unknown as IEvent<[PolkadotPrimitivesV8CandidateReceipt]>).data[0];
