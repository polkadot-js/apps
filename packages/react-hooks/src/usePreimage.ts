...
function usePreimageImpl (hashOrBounded?: Hash | HexString | FrameSupportPreimagesBounded | null): Preimage | undefined {
  ...
  const optStatus: Option<PalletPreimageRequestStatus> | undefined = useCall(!inlineData && paramsStatus && api.query.preimage?.statusFor, paramsStatus);
  ...
  // @ts-expect-error mismatch args type with `api.query.preimage?.preimageFor`
