...
export function useAccountInfo (value: string | null, isContract = false): UseAccountInfo {
  ...
  const nominator = useCall<Nominations>(api.query.staking.nominators as any, [value]);
  const validator = useCall<ValidatorPrefs>(api.query.staking.validators as any, [value]);
