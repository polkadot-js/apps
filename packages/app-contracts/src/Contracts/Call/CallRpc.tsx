...
import RpcModal, { ControlProps, ResultProps as BaseResultProps, TriggerProps } from '@polkadot/react-components/RpcModal';
...
interface ResultProps extends BaseResultProps {
...
function renderControls (props: ControlProps): React.ReactElement<ControlProps> | null {
  return null;
}

function renderResult ({ t }: Props): (_: ResultProps) => React.ReactElement<ResultProps> | null {
  return function ({ result }: ResultProps): React.ReactElement<ResultProps> | null {
    if (!result) {
      return t('Loading...');
    }
...
function CallRpc (props: Props): React.ReactElement<Props> {
  const { t } = props;
  ...
  return (
    <RpcModal
      ...
      queue={[]}
      title={t('Call a contract')}
    />
  )
}

export default translate(CallRpc);
