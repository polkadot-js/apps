...
function Sale({ chainName, coretimeInfo }: Props): React.ReactElement<Props> {
    ...
    const saleDetails = useMemo(() => calculateSaleDetails(chosenSaleNumber, saleParams?.cycleNumber, coretimeInfo.status.lastTimeslice * 80, chainName, coretimeInfo.config.regionLength, saleParams), [chosenSaleNumber, saleParams, coretimeInfo]);
    ...
    const onDropDownChange = (v: number) => setChosenSaleNumber(v)
                ...
                <div style={{ backgroundColor: 'white', borderRadius: '4px', padding: '24px', width: 'fit-content', justifySelf: 'left' }}>
                    <SummaryBox>
                        <section>
                            ...
                            <CardSummary label='last block'>{phaseName && saleParams?.phaseConfig && saleParams?.phaseConfig?.config[phaseName].lastBlock}</CardSummary>
