...
function DecisionDeposit ({ className = '', id, trackId }: Props): React.ReactElement<Props> | null {
  const { t } = useTranslation();
  const { api } = useApi();
  const [isOpen, toggleOpen] = useToggle();
  const [accountId, setAccountId] = useState<string | null>(null);

  // Get track info for the decision deposit amount
  const tracks = api.consts.referenda?.tracks;
  const track = tracks?.find(([tId]) => trackId && tId.eq(trackId))?.[1];
  const decisionDeposit = track?.decisionDeposit;
        ...
        >
          ...
          <Modal.Actions>
            <TxButton
              ...
              onStart={toggleOpen}
              params={[id]}
              tx={api.tx.referenda.placeDecisionDeposit}
            />
          </Modal.Actions>
        </Modal>
      )}
...
export default React.memo(DecisionDeposit);
