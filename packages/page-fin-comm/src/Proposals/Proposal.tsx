...
import type { AccountId, Hash, Proposal as ProposalT, Votes } from '@polkadot/types/interfaces';
...
const transformProposal = {
  transform: (optProp: Option<ProposalT>) => optProp.unwrapOr(null)
};
...
function Proposal({ className = '', imageHash, isMember, members, prime, type }: Props): React.ReactElement<Props> | null {
  const { api } = useApi();
  const proposal = useCall<ProposalT | null>(api.query.financialCommittee.proposalOf, [imageHash], transformProposal);
