...
function Apps ({ className = '' }: Props): React.ReactElement<Props> {
  ...
  const { apiEndpoint, isDevelopment, specName, systemChain, systemName } = useApi();

  const uiHighlight = useMemo(
    () => isDevelopment
      ? undefined
      : apiEndpoint?.uiColor || getSystemColor(systemChain, systemName, specName),
    [apiEndpoint, isDevelopment, specName, systemChain, systemName]
  );
