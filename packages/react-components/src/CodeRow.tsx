...
import { CodeStored } from '@polkadot/app-execute/types';
...
function CodeRow ({ buttons, children, className, code: { json }, isInline, withTags }: Props): React.ReactElement<Props> {
  ...
  const _onSaveName = useCallback(
    (): void => {
      ...
      if (trimmedName && codeHash) {
        store.saveCode(createType(registry, 'Hash', codeHash), { name })
  ...
  const _onSaveTags = useCallback(
    (): void => {
      if (codeHash) {
        store.saveCode(createType(registry, 'Hash', codeHash), { tags })
          .catch((e): void => console.error(e));
      }
    },
    [codeHash, tags]
