...
function Proposal({ className = '', imageHash, isMember, members, prime }: Props): React.ReactElement<Props> | null {
  const { api } = useApi();
  const proposal = useCall<ProposalType | null>(api.query.rootCommittee.proposalOf, [imageHash], {
    transform: (optProp: Option<ProposalType>) => optProp.unwrapOr(null)
  });
  const votes = useCall<Votes | null>(api.query.rootCommittee.voting, [imageHash], {
