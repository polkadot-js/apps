...
const DEFAULT_FLAGS_ELECTED = { withController: true, withExposure: true, withPrefs: true, withExposureMeta: true };
...
function extractSingle (api: ApiPromise, allAccounts: string[], derive: DeriveStakingElected | DeriveStakingWaiting, favorites: string[], { activeEra, eraLength, lastEra, sessionLength }: LastEra, historyDepth?: BN, withReturns?: boolean): [ValidatorInfo[], Record<string, BN>] {
  ...
  const emptyExposure = api.createType('Option<SpStakingExposurePage>');
  ...
  for (let i = 0; i < derive.info.length; i++) {
    const { accountId, exposure = emptyExposure, stakingLedger, validatorPrefs, exposureMeta } = derive.info[i];
    const expOpt = (exposure.isSome && exposure.unwrap()) || undefined;
    const expMetaOpt = (exposureMeta.isSome && exposureMeta.unwrap()) || undefined;

    // some overrides (e.g. Darwinia Crab) does not have the own/total field in Exposure
    let [bondOwn, bondTotal] = expOpt?.pageTotal && expMetaOpt?.own
      ? [expMetaOpt?.own.unwrap(), expOpt.pageTotal.unwrap()]
      : [BN_ZERO, BN_ZERO];
    ...
    const minNominated = (expOpt?.others || []).reduce((min: BN, { value = api.createType('Compact<Balance>') }): BN => {
    ...
    list[i] = {
      ...
      isNominating: (expOpt?.others || []).reduce((isNominating, indv): boolean => {
      ...
      lastPayout: earliestEra && lastEraPayout && lastEraPayout.gt(earliestEra) && !sessionLength.eq(BN_ONE)
        ...
        : undefined,
      minNominated,
      numNominators: (expOpt?.others || []).length,
