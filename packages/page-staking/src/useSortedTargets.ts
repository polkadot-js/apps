...
const transformEra = { transform: ({ activeEra }: DeriveSessionIndexes) => activeEra.gtn(0) ? activeEra.subn(1) : BN_ZERO };

const transformReward = { transform: (optBalance: Option<Balance>) => optBalance.unwrapOrDefault() };
...
export default function useSortedTargets (favorites: string[]): SortedTargets {
  ...
  const lastEra = useCall<BN>(api.derive.session.indexes, [], transformEra);
