...
function useNominationsImpl (isActive = true) {
  const { api } = useApi();
  const nominators: [StorageKey, Option<Nominations>][] | undefined = useCall(isActive && api.query.staking.nominators.entries);
