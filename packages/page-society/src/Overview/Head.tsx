// Copyright 2017-2021 @polkadot/app-society authors & contributors
// SPDX-License-Identifier: Apache-2.0

import type { DeriveSociety, DeriveSocietyMember } from '@polkadot/api-derive/types';

import React, { useMemo, useRef } from 'react';

import { Table } from '@polkadot/react-components';
import { useApi, useCall } from '@polkadot/react-hooks';

import { useTranslation } from '../translate';
import Member from './Member';

interface Props {
  className?: string;
  info?: DeriveSociety;
}

function Head ({ className = '', info }: Props): React.ReactElement<Props> {
  const { t } = useTranslation();
  const { api } = useApi();
  const members = useCall<DeriveSocietyMember[]>(api.derive.society.members);

  const headerRef = useRef([
    [t('head'), 'start', 3],
    [t('strikes')],
    []
  ]);

  const filtered = useMemo(
    () => (members || []).filter((member) => info && info.head?.eq(member.accountId)),
    [info, members]
  );

  return (
    <Table
      className={className}
      empty={info && t<string>('No current head')}
      header={headerRef.current}
    >
      {filtered.map((member): React.ReactNode => (
        <Member
          key={member.accountId.toString()}
          value={member}
        />
      ))}
    </Table>
  );
}

export default React.memo(Head);
