...
import useUniqueInfos from './useUniqueInfos';
...
function UniqueApp ({ basePath, className }: Props): React.ReactElement<Props> {
  const { t } = useTranslation();
  const { hasAccounts } = useAccounts();
  const ids = useUniqueIds();
  const infos = useUniqueInfos(ids);
...
export default React.memo(UniqueApp);
