...
function Ink3Banner ({ className = '' }: Props): React.ReactElement<Props> | null {
  ...
  return (
    <article className={`${className} warning centered`}>
      <div className='box'>
        <p>{t<string>('This contracts interface only supports Substrate 2.0 with contracts using Ink! 3.0-rc1 and newer.')}</p>
        <p>{t<string>('Older versions of the contracts ABI cannot be used and will display an error indicating an invalid ABI when uploaded. Ensure you follow the Ink! instructions for updating if you are using an older version.')}</p>
